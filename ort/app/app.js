    ...
    <div class="nav-wrapper" :class="{ 'scrolled': isScrolled }">
      <nav>
        <a href="index.html" aria-label="Domů">
          <img src="assets/logo.png" alt="ORT Logo" class="logo-img">
        </a>
...
const AppContactForm = {
  ...
  setup() {
    ...
    return { formData, isSubmitted, handleSubmit };
  },
  template: `
      ...
      <div v-reveal class="card contact-card" :class="{ 'success-state': isSubmitted }">
        <div v-if="!isSubmitted" class="contact-grid">
          ...
          <form @submit.prevent="handleSubmit" class="contact-form">
            <input type="text" v-model="formData.name" placeholder="Jméno" required>
            <input type="email" v-model="formData.email" placeholder="E-mail" required>
            <textarea v-model="formData.message" placeholder="Vaše zpráva..." rows="4" required></textarea>
...
const AppFooter = {
  template: `
    <footer>
      <img src="assets/logo.png" alt="ORT Logo" class="footer-logo-img" loading="lazy">
  ...
  setup() {
    ...
    const handleInitialHash = () => {
      if (window.location.hash) {
        ...
        if (target) {
          // Reset scroll to top before smooth scrolling to target
          window.scrollTo(0, 0);
          setTimeout(() => {
